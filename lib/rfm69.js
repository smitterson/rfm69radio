...
const rfm69 = function() {
  ...
  function sendBurstTo({ toAddress, payload }) {
    return new Promise((resolve, reject) => {
      debugPrint(`********** Sending Burst '${payload}' to address: ${toAddress}`);
      ...
      try {
        ...
        listenModeSendBurst(toAddress, payload);
        peer.sending = false;
        resolve(packet);
      } catch (err){
        reject(err);
  ...
  const DEFAULT_LISTEN_RX_US = 256;
  const DEFAULT_LISTEN_IDLE_US = 1000000; 
  let LISTEN_CYCLE_MS = (DEFAULT_LISTEN_RX_US + DEFAULT_LISTEN_IDLE_US) / 1000;

  function listenModeSendBurst(toAddress, payload) {
    //detachInterrupt(_interruptNum); TODO: remove gpio interrupt??
    setMode(reg.RF69_MODE_STANDBY);

    writeRegSync(reg.REG_PACKETCONFIG1, reg.RF_PACKET1_FORMAT_VARIABLE | reg.RF_PACKET1_DCFREE_WHITENING | reg.RF_PACKET1_CRC_ON | reg.RF_PACKET1_CRCAUTOCLEAR_ON);
    writeRegSync(reg.REG_PACKETCONFIG2, reg.RF_PACKET2_RXRESTARTDELAY_NONE | reg.RF_PACKET2_AUTORXRESTART_ON | reg.RF_PACKET2_AES_OFF);
    writeRegSync(reg.REG_SYNCVALUE1, 0x5A);
    writeRegSync(reg.REG_SYNCVALUE2, 0x5A);
    // listenModeApplyHighSpeedSettings
    writeRegSync(reg.REG_BITRATEMSB, reg.RF_BITRATEMSB_200000);
    writeRegSync(reg.REG_BITRATELSB, reg.RF_BITRATELSB_200000);
    writeRegSync(reg.REG_FDEVMSB, reg.RF_FDEVMSB_100000);
    writeRegSync(reg.REG_FDEVLSB, reg.RF_FDEVLSB_100000);
    writeRegSync(reg.REG_RXBW, reg.RF_RXBW_DCCFREQ_000 | reg.RF_RXBW_MANT_20 | reg.RF_RXBW_EXP_0);
    // end listenModeApplyHighSpeedSettings
    writeRegSync(reg.REG_FRFMSB, readRegSync(reg.REG_FRFMSB) + 1);
    writeRegSync(reg.REG_FRFLSB, readRegSync(reg.REG_FRFLSB));

    let timeRemaining = LISTEN_CYCLE_MS;

    setMode(reg.RF69_MODE_TX);
    let startTime = new Date().getTime();

    while (timeRemaining > 0) {
      //noInterrupts(); // TODO: do something here?
      //select(); //TODO: ????
      
      const bSend = [reg.REG_FIFO | 0x80, payloadStr.length + 4, toAddress, state.options.address, timeRemaining & 0xFF, (timeRemaining >> 8) & 0xFF].concat(payload);
      const message = [{
        byteLength: bSend.length,
        sendBuffer: Buffer.from(bSend),
        receiveBuffer: Buffer.alloc(bSend.length),
      ...
      while ((readRegSync(reg.REG_IRQFLAGS2) & reg.RF_IRQFLAGS2_FIFONOTEMPTY) != 0x00) {
        // make sure packet is sent before putting more into the FIFO
      }

      timeRemaining = LISTEN_CYCLE_MS - (new Date().getTime() - startTime);
    }
  ...
  return {
    ...
    sendBurst: function({ toAddress = 0, payload = '' }) {
      return sendBurstTo({ toAddress: toAddress, payload: payload });
    },
